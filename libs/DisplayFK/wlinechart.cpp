...
void LineChart::start()
{
  ...
  int16_t textSizeMin = WidgetBase::objTFT->textWidth(String(vmin).c_str());
  int16_t textSizeMax = WidgetBase::objTFT->textWidth(String(vmax).c_str());
...
void LineChart::drawBackground()
{
  ...
  int16_t maxW = WidgetBase::objTFT->textWidth(String(vmax)); // width do texto
  int16_t minW = WidgetBase::objTFT->textWidth(String(vmin)); // width do texto

  WidgetBase::objTFT->setTextDatum(TL_DATUM);
  WidgetBase::objTFT->setTextPadding(maxW);
  WidgetBase::objTFT->drawString(String(vmax), xPos + 2 + _borderSize, yPos + 2 + _borderSize); // Desenha o valor maximo

  WidgetBase::objTFT->setTextDatum(BL_DATUM);
  WidgetBase::objTFT->setTextPadding(minW);
  WidgetBase::objTFT->drawString(String(vmin), xPos + 2 + _borderSize, yPos + height - 2 - +_borderSize); // Desenha o valor minimo
#elif defined(DISP_DEFAULT)
