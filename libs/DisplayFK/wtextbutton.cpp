...
void TextButton::redraw()
{
  ...
  WidgetBase::objTFT->setTextColor(m_textColor);
        ...
        WidgetBase::objTFT->drawString(m_text, xPos + 2, yPos + (m_height / 2));
...
{
  #if defined(DISP_BODMER)
  m_width = constrain(m_width, 5, WidgetBase::objTFT->getViewportWidth());
  m_height = constrain(m_height, 5, WidgetBase::objTFT->getViewportHeight());
  #elif defined(DISP_DEFAULT)
