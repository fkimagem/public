...
void VAnalog::start()
{
  ...
  m_height = constrain(m_height, 40, WidgetBase::objTFT->getViewportHeight());
...
#if defined(DISP_BODMER)
    WidgetBase::objTFT->setTextColor(WidgetBase::lightMode ? CFK_BLACK : CFK_WHITE, WidgetBase::lightMode ? CFK_WHITE : CFK_BLACK);
    WidgetBase::objTFT->setTextPadding(m_width - 2);
    WidgetBase::objTFT->setTextDatum(BC_DATUM);
    WidgetBase::objTFT->drawString(String(m_currentValue), xPos + (m_width / 2), yPos + m_height - 2);
