...
#if defined(DISP_BODMER)
        WidgetBase::objTFT->drawString(conteudo, m_pontoPreview.x + 2, m_pontoPreview.y + (m_pontoPreview.height / 2));
...
#if defined(DISP_BODMER)
    m_screenW = WidgetBase::objTFT->getViewportWidth();
    m_screenH = WidgetBase::objTFT->getViewportHeight();
#elif defined(DISP_DEFAULT)
...
void WKeyboard::open(TextBox *_field)
{
    ...
    WidgetBase::objTFT->setFreeFont(&Roboto_Bold5pt7b);
#elif defined(DISP_DEFAULT)
    if(m_keyH > 20){
        WidgetBase::objTFT->setFont(&Roboto_Bold10pt7b);
    }else{
