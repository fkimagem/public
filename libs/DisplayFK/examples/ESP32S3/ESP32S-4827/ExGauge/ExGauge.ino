...
GaugeSuper arrayGauge[qtdGauge] = {GaugeSuper(230, 135, 0)};
const uint8_t qtdIntervalG0 = 2;
int range0[qtdIntervalG0] = {0, 50};
uint16_t colors0[qtdIntervalG0] = {CFK_AQUA, CFK_BLUE};
uint32_t count = 0;
void setup(){
    Serial.begin(115200);
...
void loop(){
    ...
    arrayGauge[0].setValue(newValue); //Use this command to change widget value.
    delay(100);
...
void loadWidgets(){
    arrayGauge[0].setup(263, 98, "Title", range0, colors0, qtdIntervalG0, 0, 100, CFK_BLACK, CFK_BLACK, CFK_WHITE, TFT_NAVY, true);
