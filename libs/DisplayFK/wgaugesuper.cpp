...
void GaugeSuper::drawBackground()
{
  ...
  if (m_divisor > 1)
  {
    ...
    WidgetBase::objTFT->setTextDatum(BR_DATUM);
    WidgetBase::objTFT->drawString("x" + String(m_divisor), xPos + (m_availableWidth / 2) - 1, yPos + (m_availableHeight / 2) - 1);
  ...
  if (m_showLabels)
  {
    ...
    for (int i = 0; i <= (2 * m_maxAngle); i += 1)
    {
      ...
      if (vFaixa >= m_intervals[m_indexCurrentStrip] && m_indexCurrentStrip < m_amountInterval)
      {
        ...
        int16_t txtWidth = WidgetBase::objTFT->textWidth(String(m_intervals[m_indexCurrentStrip] / m_divisor));
        WidgetBase::objTFT->setTextPadding(txtWidth);
        WidgetBase::objTFT->setTextDatum(alinhamento);
        WidgetBase::objTFT->drawString(String(m_intervals[m_indexCurrentStrip] / m_divisor), aX, aY);
#elif defined(DISP_DEFAULT)
        char char_arr[20];
      ...
      {
#if defined(DISP_BODMER)
        int16_t txtWidth = WidgetBase::objTFT->textWidth(String(m_vmax / m_divisor));
        WidgetBase::objTFT->setTextPadding(txtWidth);
        WidgetBase::objTFT->setTextDatum(BL_DATUM);
        WidgetBase::objTFT->drawString(String(m_vmax / m_divisor), x0, y0);
#elif defined(DISP_DEFAULT)
...
void GaugeSuper::redraw()
{
  ...
  {
#if defined(DISP_BODMER)
    WidgetBase::objTFT->setTextPadding(50);
    WidgetBase::objTFT->setTextDatum(BL_DATUM);
    WidgetBase::objTFT->drawString(buf, xPos - (_availableWidth / 2) + 1, yPos + (_availableHeight / 2) - 1); // Desenha o value
#elif defined(DISP_DEFAULT)
...
#if defined(DISP_BODMER)
    int16_t txtWidth = WidgetBase::objTFT->textWidth(_title);
    WidgetBase::objTFT->setTextPadding(txtWidth);
    WidgetBase::objTFT->setTextDatum(TC_DATUM);
    WidgetBase::objTFT->drawString(m_title, xPos, yPos - (m_availableHeight / 4) + 1); // // Redesenha o title depois que a linha mudou de lugar
    // WidgetBase::objTFT->unloadFont();
