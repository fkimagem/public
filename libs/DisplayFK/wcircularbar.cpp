...
void CircularBar::redraw()
{
    ...
    WidgetBase::objTFT->drawArc(xPos, yPos, m_radius, (m_radius - m_lineWeight), angleValue, lastAngleValue, m_bkColor, m_bkColor);
    WidgetBase::objTFT->drawArc(xPos, yPos, m_radius, (m_radius - m_lineWeight), 0, angleValue, m_lineColor, m_bkColor);
...
#if defined(DISP_BODMER)
        int16_t txtWidth = WidgetBase::objTFT->textWidth(char_arr);
        WidgetBase::objTFT->setTextPadding(txtWidth);
        WidgetBase::objTFT->setTextDatum(MC_DATUM);
        WidgetBase::objTFT->drawString(char_arr, xPos, yPos);
#elif defined(DISP_DEFAULT)
