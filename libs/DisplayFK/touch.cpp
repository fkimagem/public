...
void TouchScreen::drawStar(int16_t xPos, int16_t yPos, uint8_t size, uint16_t color)
{
  if (m_objTFT)
  {
    /*uint16_t cor1 = 0xf800;
    uint16_t cor2 = 0x07c0;
    uint16_t cor3 = 0x001f;
    uint16_t cor4 = 0x07ff;
    uint16_t cor5 = 0xf81f;
    uint16_t cor6 = 0xffe0;*/

    /*objTFT->drawLine(xPos-1, yPos - size, xPos-1, yPos + size, cor1);
    m_objTFT->drawLine(xPos, yPos - size, xPos, yPos + size, cor1);
    m_objTFT->drawLine(xPos+1, yPos - size, xPos+1, yPos + size, cor1);


    m_objTFT->drawLine(xPos - size, yPos - size, xPos + size, yPos + size, cor2);

    m_objTFT->drawLine(xPos - size, yPos - 1, xPos + size, yPos - 1, cor3);
    m_objTFT->drawLine(xPos - size, yPos, xPos + size, yPos, cor3);
    m_objTFT->drawLine(xPos - size, yPos + 1, xPos + size, yPos + 1, cor3);

    objTFT->drawLine(xPos - size, yPos + size, xPos + size, yPos - size, cor4);*/
...
ScreenPoint_t TouchScreen::getScreenPosition(int16_t xTouch, int16_t yTouch)
{
  ...
  case 2:
  {
    minX = extremeMin.xScreen;
    maxX = extremeMax.xScreen;
    minY = extremeMin.yScreen;
    maxY = extremeMax.yScreen;

    screenPos.x = map(xTouch, m_calibMatrix[0].xTouch, m_calibMatrix[2].xTouch, minX, maxX);
    screenPos.y = map(yTouch, m_calibMatrix[0].yTouch, m_calibMatrix[2].yTouch, minY, maxY);
  }
  break;
  case 1:
  case 3:
  {
    ...
    screenPos.y = map(xTouch, extremeMin.xTouch, extremeMax.xTouch, minY, maxY);
    screenPos.x = map(yTouch, extremeMin.yTouch, extremeMax.yTouch, minX, maxX);
  }
  break;
  ...
  /*if(rotation == 1 || rotation == 3){//Para o getTouch normal usar 1 || 3.
    screenPos.x = map(xTouch, calibMatrix[0].xTouch, calibMatrix[2].xTouch, calibMatrix[0].xScreen, calibMatrix[2].xScreen);
    screenPos.y = map(yTouch, calibMatrix[0].yTouch, calibMatrix[2].yTouch, calibMatrix[0].yScreen, calibMatrix[2].yScreen);
  }else{
    screenPos.y = map(xTouch, calibMatrix[0].xTouch, calibMatrix[2].xTouch, calibMatrix[0].yScreen, calibMatrix[2].yScreen);
    screenPos.x = map(yTouch, calibMatrix[0].yTouch, calibMatrix[2].yTouch, calibMatrix[0].xScreen, calibMatrix[2].xScreen);
  }*/
