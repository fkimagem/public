...
void Label::redraw()
{
  ...
  WidgetBase::objTFT->setFreeFont(m_fontFamily);
  WidgetBase::objTFT->setTextColor(m_letterColor);
  int16_t maxW = WidgetBase::objTFT->textWidth(m_previousText); // Largura do texto
  WidgetBase::objTFT->setTextDatum(m_datum);
  WidgetBase::objTFT->setTextPadding(maxW);
  WidgetBase::objTFT->drawString(m_text, xPos, yPos);
