...
void SpinBox::redraw()
{
    ...
    WidgetBase::objTFT->setFreeFont(GetBestRobotoBold(btnH));

    int16_t maxW = WidgetBase::objTFT->textWidth(String(m_vmax));
    WidgetBase::objTFT->setTextPadding(maxW);

    WidgetBase::objTFT->setTextDatum(MC_DATUM);
    WidgetBase::objTFT->drawString(String(m_currentValue), xPos + m_width / 2, yPos + (m_height / 2) - 3); // Desenha o numer
